...
describe('Placement.groupHead', () => {
    describe('a row that is not indented', () => {
        it('is its own head below a heading', () => {
            expect(Placement.groupHead(['## Tasks', '- [ ] first', '- [ ] second'], 2).at).toBe(1);
        });

        it('climbs the tasks just above it', () => {
            expect(Placement.groupHead(['## Tasks', '- [x] older', '- [x] middle', '- [ ] current'], 3).at).toBe(1);
        });
        ...
        it('climbs over the children of the siblings above', () => {
            ...
            expect(Placement.groupHead(lines, 5).at).toBe(1);
        });

        it('stops at a blank line between two siblings', () => {
            expect(Placement.groupHead(['- [x] unrelated', '', '- [x] group start', '- [ ] current'], 3).at).toBe(2);
        });

        it('is line 0 when it is the whole note', () => {
            expect(Placement.groupHead(['- [ ] only task'], 0).at).toBe(0);
        });
        ...
        it('does not go into the fence just above it', () => {
            ...
            expect(Placement.groupHead(lines, 4).at).toBe(4);
        });
        ...
        it('does not go above the frontmatter when it is the first line of the body', () => {
            ...
            expect(Placement.groupHead(lines, 3).at).toBe(3);
        });

        it('climbs siblings up to the frontmatter and no further', () => {
            const lines = ['---', 'tags: a', '---', '- [x] first', '- [ ] current'];
            expect(Placement.groupHead(lines, 4).at).toBe(3);
        });

        it('stops at a --- rule', () => {
            expect(Placement.groupHead(['- [x] above', '---', '- [ ] current'], 2).at).toBe(2);
        });

        it('stops at a paragraph', () => {
            expect(Placement.groupHead(['Some text', '- [ ] current'], 1).at).toBe(1);
        });

        it('stops at a table', () => {
            expect(Placement.groupHead(['| a | b |', '| - | - |', '| 1 | 2 |', '- [ ] current'], 3).at).toBe(3);
        });

        it('stops at a list item that is not a task', () => {
            expect(Placement.groupHead(['- note', '- [ ] current'], 1).at).toBe(1);
        });

        it('stops at a paragraph whose continuation is indented', () => {
            expect(Placement.groupHead(['paragraph', '    deeper text', '- [ ] current'], 2).at).toBe(2);
        });
    });

    describe('a row that is indented', () => {
        it('goes just below its parent', () => {
            expect(Placement.groupHead(['- [ ] parent', '    - [x] child1', '    - [ ] child2'], 2).at).toBe(1);
        });

        it('goes just below its parent past other siblings', () => {
            const lines = ['- [ ] parent', '    - [x] child1', '    - [x] child2', '    - [ ] child3'];
            expect(Placement.groupHead(lines, 3).at).toBe(1);
        });
...
describe('Placement.afterSubtree', () => {
    it('is past the children, a blank line inside them included, and before the blank lines after', () => {
        const lines = ['- [ ] a', '\t- b', '', '\t- c', '', '- [ ] d'];
        expect(Placement.afterSubtree(lines, 0).at).toBe(4);
    });
    ...
    it('answers where the rule lands even inside a fence at the top that never closes', () => {
        ...
        expect(Placement.afterSubtree(lines, 3).at).toBe(4);
...
describe('Placement.firstChild', () => {
    it('is the line below the row, under it, at its children\'s indentation', () => {
        expect(Placement.firstChild(['- [ ] a', '\t- b'], 0)).toEqual({ at: 1, parent: 0, indent: '\t' });
    });
    ...
    it('is past the row\'s text that goes on, which a child put above it would take in (P1)', () => {
        ...
        expect(Placement.firstChild(['- [ ] a', 'lazy', '- [ ] b'], 0).at).toBe(2);
        expect(Placement.firstChild(['- [ ] a', '      deep', '  more', '- [ ] b'], 0).at).toBe(3);
        // Not past a blank line, a fence or a child: those start a block of
        // their own below the child put.
        expect(Placement.firstChild(['- [ ] a', '', '  para'], 0).at).toBe(1);
        expect(Placement.firstChild(['- [ ] a', '  ```', '  x', '  ```'], 0).at).toBe(1);
        expect(Placement.firstChild(['- [ ] a', 'text', '  - b'], 0).at).toBe(2);
    });
});

describe('Placement.lastChild', () => {
    it('is past the row\'s subtree, under the row, at its children\'s indentation', () => {
        expect(Placement.lastChild(['- [ ] a', '  - b', '    - c', '- [ ] d'], 0)).toEqual({ at: 3, parent: 0, indent: '  ' });
        // No child to copy: the file's unit, to the row's content column.
        expect(Placement.lastChild(['100. [ ] a', '- x', '\t- y'], 0)).toEqual({ at: 1, parent: 0, indent: '\t\t' });
    });
});

describe('Placement.before', () => {
    it('is the row\'s own line, as its sibling', () => {
        expect(Placement.before(['- [ ] p', '\t- [ ] a', '\t\t- c'], 1)).toEqual({ at: 1, parent: 0, indent: '\t' });
    });
});

describe('Placement.underHeading', () => {
    it('is just below the heading, at the top, unindented when no item follows', () => {
        expect(Placement.underHeading(['## H', '- [ ] a'], 0)).toEqual({ at: 1, parent: null, indent: '' });
        expect(Placement.underHeading(['## H'], 0)).toEqual({ at: 1, parent: null, indent: '' });
    });

    it('is at the indentation of the first item at the top below it, blank lines aside, as its sibling (P1)', () => {
        expect(Placement.underHeading(['## H', '', '  - [ ] a', '\t- [ ] b'], 0)).toEqual({ at: 1, parent: null, indent: '  ' });
    });

    it('is past the paragraph and the indented code below the heading, and not past the next heading', () => {
        expect(Placement.underHeading(['## H', 'para', 'more', '', '- [ ] a'], 0).at).toBe(3);
        // Four columns under a heading is indented code (measurement.md q10).
        expect(Placement.underHeading(['## H', '\t- [ ] a', '\t\t- [ ] b', '- [ ] c'], 0)).toEqual({ at: 3, parent: null, indent: '' });
        expect(Placement.underHeading(['## H', '### Sub', 'para'], 0).at).toBe(1);
        expect(Placement.underHeading(['## H', '---', 'para'], 0).at).toBe(1);
    });
});
...
describe('the spot\'s parent and indentation', () => {
    it('is the row\'s parent and the row\'s indentation for a sibling', () => {
        ...
        expect(Placement.groupHead(lines, 2)).toEqual({ at: 1, parent: 0, indent: '    ' });
        expect(Placement.afterSubtree(lines, 1)).toEqual({ at: 2, parent: 0, indent: '    ' });
        expect(Placement.afterCompletedRun(lines, 1)).toEqual({ at: 2, parent: 0, indent: '    ' });
        expect(Placement.groupHead(lines, 3)).toEqual({ at: 0, parent: null, indent: '' });
    ...
    it('goes past the parent\'s own text that goes on, for the head of a group under it', () => {
        expect(Placement.groupHead(['- [ ] p', '  text', '  - [ ] a'], 2)).toEqual({ at: 2, parent: 0, indent: '  ' });
    });
});
