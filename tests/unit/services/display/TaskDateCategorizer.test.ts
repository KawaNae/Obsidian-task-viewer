...
describe('日付所属: allDay', () => {
    // NOTE: 現行の calendar 範囲比較を pin する characterization テスト。
    // S-AllDay は effective 解決で翌日 04:59 の endDate を持つため、
    // AllDay レーン（visual 基準で 1 日）と異なり calendar 2 日分に所属する。
    // 本シリーズの後続コミットで visual 基準（getTaskDateRange）に変更予定。
    it('S-AllDay Jan15 は Jan15 と Jan16 の両方に所属する（現行 calendar 基準）', () => {
        ...
        expect(idsOf(categorizeTasksForDate([task], '2026-01-16', startHour).allDay)).toEqual(['allday-s']);
        expect(idsOf(categorizeTasksForDate([task], '2026-01-17', startHour).allDay)).toEqual([]);
    });

    it('SE-AllDay Jan15〜Jan17 は Jan15/16/17 に所属する（現行 calendar 基準）', () => {
        const task = dt({ id: 'allday-se', startDate: '2026-01-15', endDate: '2026-01-17' });
        for (const date of ['2026-01-15', '2026-01-16', '2026-01-17']) {
            expect(idsOf(categorizeTasksForDate([task], date, startHour).allDay)).toEqual(['allday-se']);
        }
        expect(idsOf(categorizeTasksForDate([task], '2026-01-18', startHour).allDay)).toEqual([]);
    });

    it('反転 range（effectiveEndDate < effectiveStartDate）はどの日付にも所属しない（現行の消失挙動）', () => {
...
describe('categorizeTasksForDate ≡ categorizeTasksByDate（単日と複数日の一致性）', () => {
    ...
    it('複数日の分配 snapshot', () => {
        ...
        expect(snapshot).toEqual([
            ...
            '2026-01-16 allDay=[f-se,f-allday] timed=[] dueOnly=[f-due]',
...
describe('バケツ内ソート（TaskRenderOrder 準拠）', () => {
    ...
    it('allDay: 開始日昇順、同日は id 昇順', () => {
        ...
        const allDay = categorizeTasksForDate([b, a, earlier], '2026-01-16', startHour).allDay;
