...
import type { StaticType } from '../services/lang/functions';
import { parseFlowCells } from '../services/flow/FlowParser';
import {
    joinSegments,
...
/**
 * Every state cell the file declares, by the type it starts from.
 *
 * A block is checked without knowing which command fires it — the name in
 ...
 * a squiggle under a name that is a cell would be the editor calling correct
 * work wrong.
 *
 * Read per line rather than per command: a multi-line command puts its
 * clauses on lines of their own, and a line carrying only `let(...)` is not a
 * program — the declaration on it is true all the same.
 */
function declaredCells(lines: string[], fenced: boolean[], dedented: boolean[]): GenCellTypes {
    const types = new Map<string, StaticType>();
    lines.forEach((text, i) => {
        if (fenced[i] || dedented[i]) return;
        const tail = flowTailOf(text);
        if (tail === null) return;
        for (const cell of parseFlowCells(tail)) {
            // A literal is never a list or a record, and a type this map has
            // no word for would be a claim about the cell that is not true.
...
export function createDiagnosticsExtension(): Extension {
    ...
    const analyze = (doc: Text): DocAnalysis => {
        ...
        const cells = blocks.size > 0 ? declaredCells(lines, scan.fenced, dedented) : undefined;
