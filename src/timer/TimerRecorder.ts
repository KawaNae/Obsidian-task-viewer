...
/** 開始を押したときの対象の写しと、開始の書き込みでその行に付ける `^id`。 */
...
export class TimerRecorder {
    ...
    /**
     * 開始を押したときの対象の写しと、その行に付ける `^id`（行に錨があれば無し）。
     * タイマーの対象の錨はここで決まる。
     *
    ...
    private startTarget(timer: TimerInstance): StartTarget | null {
        ...
        if (task.anchor) {
            timer.timerTargetId = task.anchor;
            timer.autoGeneratedTargetId = this.storageUtils.isAutoManagedTimerTargetId(task.anchor);
            return { task };
        }
        if (task.blockId) {
     ...
     */
    private async startOnTarget(timer: TimerInstance, { task, rowId }: StartTarget): Promise<boolean> {
        const now = new Date();
    ...
    private async releaseTailId(
        ...
        // 対象の錨は対象の片付け（TimerTargetManager.cleanupGeneratedTargetId）が外す。
        if (blockId === timer.timerTargetId) return;
    ...
    async discardRunningPlaceholder(timer: TimerInstance): Promise<void> {
        ...
        // self の 1 本目は対象の行そのもの。行も錨も消さない（錨は閉じるときに片付く）。
        if (blockId === timer.timerTargetId) return;
