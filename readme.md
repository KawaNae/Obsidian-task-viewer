...
プラグインは2つのイベントの発火順序を監視することで、変更がローカル操作か同期によるものかを判定する。

```
ローカル操作の場合:
  editor-change → vault.modify
  (ユーザーがエディタで編集 → ファイルが保存される)

同期による変更の場合:
  vault.modify → editor-change (または editor-change なし)
  (外部からファイルが更新される → エディタに反映される)
```
